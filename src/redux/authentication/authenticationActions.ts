...
const registerUser = (
    ...
    try {
        ...
        await authHttp('https://datagrid-express.herokuapp.com/api/auth/register', 'POST', {
            ...userObject,
        });
        const data = await authHttp(
            'https://datagrid-express.herokuapp.com/api/auth/login',
            'POST',
            { ...userObject }
        );
...
const loginUser = (
    ...
    try {
        dispatch(fetchAuthPending());
        const tokenData = await authHttp(
            'https://datagrid-express.herokuapp.com/api/auth/login',
            'POST',
            { ...userObject }
        );
